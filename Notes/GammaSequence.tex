...
% Given some $\pazocal F \in S_f$, we define $\tau(\pazocal F)$ to the number $i \in [m]$ such that $\underline{\pazocal F}_i$ is the first insertion that causes a node in $\{y_i\}_{i=l_1+1}^{l_2}$ to be activated.
...
%     \item $\pazocal F^\star$ can be obtained by starting with $\pazocal F$ and permuting the positions of the edges in $\pazocal F$ that appear strictly before the edge $\underline{\pazocal F}_i$ with $i=\tau(\pazocal F)$ and are contained in $B$.
% \end{enumerate}
...
% Now let $\pazocal F^\star \in S_f$ and let $P=[\pazocal F^\star]_{\simeq}$. We will see that $\mathbb E_{\pazocal F \in P}[\Lambda_t(\underline{\pazocal F}, \prec_0)] = \polylog n$. This will follows from the following facts.
